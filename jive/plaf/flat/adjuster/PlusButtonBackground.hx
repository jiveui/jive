...
class PlusButtonBackground implements GroundDecorator implements UIResource {
	...
	public function updateDecorator(c:Component, g:Graphics2D, bounds:IntRectangle) {
		...
		g.lineTo(bounds.x + bounds.width - cornerRadius, bounds.y);
		g.curveTo(bounds.x + bounds.width, bounds.y, bounds.x + bounds.width, bounds.y + cornerRadius);
