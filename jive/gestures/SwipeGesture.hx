...
class SwipeGesture extends Gesture
{
    ...
    private static inline var MAX_DURATION:UInt = 200;
    ...
    override public function setState(newState:GestureState):Bool 
    {
        if (newState == GestureState.FAILED) {
            var stack = CallStack.toString(CallStack.callStack()).split('\n');
            for(s in stack)
                trace(s);
