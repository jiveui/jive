...
class Color {
    ...
    public static var LIGHT_GRAY:Color = new Color(0xc0c0c0);

    public static var GRAY:Color = new Color(0x808080);

    public static var DARK_GRAY:Color = new Color(0x404040);

    public static var BLACK:Color = new Color(0x000000);

    public static var RED:Color = new Color(0xff0000);

    public static var PINK:Color = new Color(0xffafaf);

    public static var ORANGE:Color = new Color(0xffc800);

    public static var HALO_ORANGE:Color = new Color(0xFFC200);

    public static var YELLOW:Color = new Color(0xffff00);

    public static var GREEN:Color = new Color(0x00ff00);

    public static var HALO_GREEN:Color = new Color(0x80FF4D);

    public static var MAGENTA:Color = new Color(0xff00ff);

    public static var CYAN:Color = new Color(0x00ffff);

    public static var BLUE:Color = new Color(0x0000ff);

    public static var HALO_BLUE:Color = new Color(0x2BF5F5);
    ...
    /**
	 * Create a new <code>Color</code> with another alpha but same rgb.
	 * @param newAlpha the new alpha
	 * @return the new <code>Color</code>
	 */

    public function changeAlpha(newAlpha:Float):Color {
        return new Color(rgb, newAlpha);
    }

    /**
	 * Create a new <code>Color</code> with just change hue channel value.
	 * @param newHue the new hue value
	 * @return the new <code>Color</code>
	 */

    public function changeHue(newHue:Float):Color {
        return getColorWithHLS(newHue, getLuminance(), getSaturation(), alpha);
    }

    /**
	 * Create a new <code>Color</code> with just change luminance channel value.
	 * @param newLuminance the new luminance value
	 * @return the new <code>Color</code>
	 */

    public function changeLuminance(newLuminance:Float):Color {
        return getColorWithHLS(getHue(), newLuminance, getSaturation(), alpha);
    }

    /**
	 * Create a new <code>Color</code> with just change saturation channel value.
	 * @param newSaturation the new saturation value
	 * @return the new <code>Color</code>
	 */

    public function changeSaturation(newSaturation:Float):Color {
        return getColorWithHLS(getHue(), getLuminance(), newSaturation, alpha);
    }

    /**
	 * Create a new <code>Color</code> with just scale the hue luminace and saturation channel values.
	 * @param hScale scale for hue
	 * @param lScale scale for luminance
	 * @param sScale scale for saturation
	 * @return the new <code>Color</code> scaled
	 */

    public function scaleHLS(hScale:Float, lScale:Float, sScale:Float):Color {
        var h:Float = getHue() * hScale;
        var l:Float = getLuminance() * lScale;
        var s:Float = getSaturation() * sScale;
        return getColorWithHLS(h, l, s, _alpha);
    }

    /**
	 * Create a new <code>Color</code> with just offset the hue luminace and saturation channel values.
	 * @param hOffset offset for hue
	 * @param lOffset offset for luminance
	 * @param sOffset offset for saturation
	 * @return the new <code>Color</code> offseted
	 */

    public function offsetHLS(hOffset:Float, lOffset:Float, sOffset:Float):Color {
    ...
    public function darker(factor:Float = 0.7):Color {
        var r:Int = getRed();
        var g:Int = getGreen();
        var b:Int = getBlue();
        return getColor(Std.int(r * factor), Std.int(g * factor), Std.int(b * factor), _alpha);
    }
    ...
    public function brighter(factor:Float = 0.7):Color {
        var r:Float = getRed();
        ...
        if (r == 0 && g == 0 && b == 0) {
            return getColor(Std.int(i), Std.int(i), Std.int(i), _alpha);
        }
        if (r > 0 && r < i) r = i;
        if (g > 0 && g < i) g = i;
        if (b > 0 && b < i) b = i;
    ...
    /**
	 * Returns a Color with the specified red, green, blue values in the range [0 - 255] 
    ...
    public static function getColor(r:Int, g:Int, b:Int, a:Float = 1):Color {
        return new Color(getRGBWith(r, g, b), a);
    }
    ...
    public static function getWithARGB(argb:Int):Color {
    ...
    /**
	 * Returns a Color with with the specified hue, luminance, 
    ...
    public static function getColorWithHLS(h:Float, l:Float, s:Float, a:Float = 1):Color {
        var c:Color = new Color(0, a);
        c.hlsCounted = true;
    ...
    public function toString():String {
        return "Color(rgb:" + StringTools.hex(_rgb) + ", alpha:" + _alpha + ")";
    }
    ...
    /**
	 * Clone a Color, most time you dont need to call this because Color 
    ...
    public static function getColorBetween(begin:Color, end:Color, distance:Float):Color {
