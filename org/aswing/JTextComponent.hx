 ...
 * @see JTextArea
 */
class JTextComponent extends Component  implements EditableComponent implements bindx.IBindable {
    ...
    **/
    ...
    public function new(){
		...
		addChild(textField);
		textField.addEventListener(Event.CHANGE, function(e) { bindx.Bind.notify(this.text, null, this.text);});
	...
	public function setText(text:String):Void{
		...
		if(getTextField().text != text){
			var oldText = getTextField().text;
			getTextField().text = text;
			if(isAutoSize()){
				revalidate();
			}
			bindx.Bind.notify(this.text, oldText, text);
		}
	}
